 ...
 */
public class RobotSwerveDriveCommandTest extends TestHelper {

	@Test
	public void testCommandFile() throws IOException {
		String filename = testdir + "/swerve_test.txt";
		List<JoystickInput> commands = Commands.readFileSingleInput(filename);

		Robot robot = new Robot(new SwerveRobotDrive(table, 0, 0, Facing.EAST));
		robot.execute(commands);
		assertEquals("(0,0) EAST", robot.getDrive().toString());
	}
}
